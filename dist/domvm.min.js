...
function(g,u){function t(){var b=location,b=v?b.hash.substr(1)||"/":b.href.substr(b.origin.length);return z(D,k,b)}var h=null,q={href:function(b,e,h,g,r){var t=z(D,k,b,e,h,g);if(v)return"#"+t.href;b=function(b){q["goto"](t,null,null,null,r);b.preventDefault()};b.href=t.href;return b},config:function(b){(v=b.useHash)||(k=b.root||"");h=b.init||null},refresh:function(){q["goto"](t(),null,null,null,!0)},"goto":function(f,h,g,q,r){r=r||w;f.href||(f=z(D,k,f,h,g,q));if(!1===f.name)console.log("Could not find route");
else{h=null;for(g=0;g<e.length;g++)if(e[g].path===f.path){h=g;break}null===h&&(e.splice(b+1,1E4),e.push(f),h=e.length-1);f=e[b];g=e[h];q=!0;null!==b&&(q=(q=D[f.name].onexit)?q.apply(null,(f?[f.segs,f.query,f.hash]:[]).concat({to:g})):!0);if(!1!==q&&!1!==D[g.name].onenter.apply(null,(g?[g.segs,g.query,g.hash]:[]).concat({from:f}))){if(!v)history[r?"replaceState":"pushState"](null,"title",g.href);b=h}w=!1}},current:function(){return w?t():e[b]}},w=!0,D=g(q,u);r(D,k);onhashchange=onpopstate=function(b){v&&
