...
var n,a,m;return function(){a=this;m=arguments;n||(n=r(d))}}};w.utils=f})(domvm);(function(w){function r(){for(var b,g,k=[I,L];g=k.shift();)for(;b=g.shift();)b[0].apply(null,b.slice(1))}function y(b,g){return!1===g?null:null==g&&null!=b?b:l.isVal(g)||l.isObj(g)||l.isArr(g)||l.isFunc(g)?g:null}function f(b,g,k,a,A,e){function t(b,g,e){if(e)b[g]=b[g]||[],b[g].push(e);else for(var k in g)b[k]=b[k]||[],b[k].push(g[k])}function v(b,g,k){A=b;e=g;q.update(k,!1);null!=q.node&&(A.body[e]=q.node);return f(0,!1)}function u(b){return null==b?"":b}function f(b,c){if((c=!1!==c)&&I.length)return L.push([f,
...
b.vm.hooks&&!b.moved&&d(b.vm,"didMount",b.vm);return a!==b.el?a:a.nextSibling}function C(b,g,a,c,h){var e=l.isArr(b),d=e?y(b[1],b[2]):b.key,m=a.hasKeys,v=g.hasKeys;a=a.body;g=g.body;if(null!==d&&m)return b=l.keyedIdx(d,a,e?b[0]:null),-1<b?[b,2]:null;c=c||0;if(c>a.length-1)return null;h=0===h?0:h||a.length-1;for(d=null;c<=h;c++)if(m=a[c],null!==m&&m.el)if(e&&m.vm){if(m.vm.view[0]===b[0]){if(m.vm.view[1]===y(b[1],b[2]))return[c,2];!d&&v&&-1==l.keyedIdx(m.key,g,m.vm.view[0])&&(d=[c,1])}}else if(b.type===
...
a,null,f)},"delete":function(a,d,f){f?a+=n.query(d):f=d;return c("delete",a,null,f)},post:function(a,d,f){return c("post",a,d,f)},put:function(a,d,f){return c("put",a,d,f)},patch:function(a,d,f){return c("patch",a,d,f)},query:function(a){if(!r.isObj(a))return"";var c=[],d=encodeURIComponent,f;for(f in a)c.push(d(f)+"="+d(a[f]));return c.length?"?"+c.join("&"):""}};return n}})(domvm);(function(w){function r(a,c,d,f,m,n){var v=null,h=null,r=null;if("/"==d[0]){r=d;h=!1;f={};n=r.split("#");c=n[0].split("?");v=c[0];n=n[1];c[1]&&(m={},c[1].split("&").map(function(a){a=a.split("=");m[a[0]]=null==a[1]?!0:a[1]}));var y,x;for(x in a){d=a[x];var w=d.path;if(y=v.match(d.regexPath)){h=x;-1!==w.indexOf(":")&&(f={},y.shift(),w.replace(/:([^\/]+)/g,function(a,c){f[c]=y.shift()}));break}}}else{h=d;d=a[h];var w=c+d.path,H=d.vars||{},r=-1!==w.indexOf(":")?v=w.replace(/:([^\/]+)/g,function(a,c){if((H[c]||
/^[^\/]+$/).test(f[c]))return f[c]+="";throw Error("Invalid value for route '"+w+"' segment '"+c+"': '"+f[c]+"'");}):v=w;if(m){var r=r+"?",F;for(F in m)r+=F+(!0!==m[F]?"="+m[F]:"")+"&";r=r.slice(0,-1)}r+=n?"#"+n:""}return{href:r,name:h,path:v,segs:f,hash:n,query:m}}function y(a,c){for(var d in a){var f=a[d];f.regexPath=new RegExp("^"+c+f.path.replace(/:([^\/]+)/g,function(a,c){var d=""+((f.vars||{})[c]||/[^\/]+/);return"("+d.substring(1,d.lastIndexOf("/"))+")"})+"$")}}var f=[],c=null,d=!1,n=null,
a=null,m="";w.route=function(u,w){function p(){var a=location,a=d?a.href.substr(a.origin.length):a.hash.substr(1)||"/";return r(B,m,a)}var C=null,D={href:function(a,c,f,n,p){var v=r(B,m,a,c,f,n);return d?(a=function(a){D["goto"](v,null,null,null,p);a.preventDefault()},a.href=v.href,a):"#"+v.href},config:function(c){(d=c.useHist)&&(m=c.root||"");n=c.willEnter||null;a=c.willExit||null;C=c.init||null},refresh:function(){D["goto"](p(),null,null,null,!0)},"goto":function(h,p,u,x,w){h.href||(h=r(B,m,h,
p,u,x));if(!1===h.name)console.log("Could not find route");else{p=null;for(x=0;x<f.length;x++)if(f[x].href===h.href){p=x;break}null===p&&(f.splice(c+1,1E4),f.push(h),p=f.length-1);h=f[c];x=f[p];u=!0;null!==c&&(a&&a(h,x),u=(u=B[h.name].onexit)?u.apply(null,(h?[h.segs,h.query,h.hash]:[]).concat(x)):!0);!1!==u&&(n&&n(x,h),!1!==B[x.name].onenter.apply(null,(x?[x.segs,x.query,x.hash]:[]).concat(h))&&(d?(v=!0,history[w?"replaceState":"pushState"](null,"title",x.href)):(x="#"+x.href,location.hash!==x&&(v=
!0,w?location.replace(x):location.hash=x)),c=p))}},location:function(){return null==c?p():f[c]}},B=u(D,w);y(B,m);var v=!1;window.onhashchange=window.onpopstate=function(a){if(d||"popstate"!=a.type)!d&&v?v=!1:D["goto"](p(),null,null,null,!0)};C&&C();return D}})(domvm);
