...
else{p=null;for(q=0;q<a.length;q++)if(a[q].path===m.path){p=q;break}null===p&&(a.splice(c+1,1E4),a.push(m),p=a.length-1);m=a[c];q=a[p];r=!0;null!==c&&(r=(r=D[m.name].onexit)?r.apply(null,(m?[m.segs,m.query,m.hash]:[]).concat(q)):!0);if(!1!==r&&!1!==D[q.name].onenter.apply(null,(q?[q.segs,q.query,q.hash]:[]).concat(m))){if(x)history[u?"replaceState":"pushState"](null,"title",q.href);c=p}y=!1}},current:function(){return y?r():a[c]}},y=!0,D=p(q,v);u(D,l);onhashchange=onpopstate=function(a){(x||"popstate"!=
